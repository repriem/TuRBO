...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr  1 14:08:45 2020
...
class TurboM(Turbo1):
    ...
    def __init__(
        ...
        file = open('Data_TurBO-M_%s_in_%i_dimensions' %(func,dim), 'w')
        file.write("Using dtype = %s \r Using device = %s \r Number of initial points = %i \r Total evaluation budget= %i" %(dtype,device, n_init, max_evals))
    ...
    def optimize(self):
        ...
        file = open('Data_TurBO-M_%s_in_%i_dimensions' %(func,dim), 'a')
        ...
        for i in range(self.n_trust_regions):  
            ...
            if self.verbose:
                ...
                file.write(f"\r TR-{i} starting from: {fbest:.4}")
                sys.stdout.flush()
            ...
            for i in range(self.n_trust_regions):
                ...
                if len(idx_i) > 0:
                    ...
                    if self.verbose and fX_i.min() < self.fX.min() - 1e-3 * math.fabs(self.fX.min()):
                        ...
                        file.write(f"\r {n_evals}) New best @ TR-{i}: {fbest:.4}")
                        sys.stdout.flush()
            ...
            for i in range(self.n_trust_regions):
                if self.length[i] < self.length_min:  # Restart trust region if converged
                    ...
                    if self.verbose:
                        ...
                        file.write(f"\r {n_evals}) TR-{i} converged to: : {fbest:.4}")
                        sys.stdout.flush()
                    ...
                    if self.verbose:
                        ...
                        file.write(f"\r {n_evals}) TR-{i} is restarting from: : {fbest:.4}")
                        sys.stdout.flush()
...
class Rosenbrock:
    ...
    def __call__(self, x):
        ...
        assert x.ndim == 1
        assert np.all(x <= self.ub) and np.all(x >= self.lb)
        val = np.sum(100*((x[2:self.dim])-(x[1:self.dim - 1])**2)**2+(1-(x[1:self.dim - 1]))**2) 
        return val  
...
class Rastrigin:
    ...
    def __call__(self, x):
        ...
        assert x.ndim == 1
        assert np.all(x <= self.ub) and np.all(x >= self.lb)
        val = 10*self.dim + np.sum((x[1:self.dim])**2-10*np.cos(2*np.pi*(x[1:self.dim])))
        return val  
...
if __name__=='__main__':
    ...
    turbo_m = TurboM(
        ...
        n_init=24,  # Number of initial bounds from an Symmetric Latin hypercube design
        max_evals=380,  # Maximum number of evaluations
    ...
    plt.ylabel('Result')
    plt.title("Function %s in %i dimensions" %(func, dim))
    plt.tight_layout()
    plt.show()  
    #Saving every data in a txt file, the plot in a pdf file and fX in a npy file
    fig.savefig('Plot_TurBO-M_%s_in_%i_dimensions' %(func,dim),format='pdf', dpi=250)
    np.save('fX_TurBO-M_%s_in_%i_dimensions' %(func,dim),fX)
    np.save('X_TurBO-M_%s_in_%i_dimensions' %(func,dim),X)
